...
class RBACTests(unittest.TestCase):
    ...
    @mock.patch("kerlescan.rbac_service_interface.get_rbac_filters")
    @mock.patch("kerlescan.view_helpers.get_key_from_headers")
    @mock.patch("kerlescan.rbac_service_interface.fetch_url")
    def test_rbac_calls_store_rbac_filters(
        self, mock_fetch_url, mock_get_key, mocked_get_rbac_filters
    ):
        mock_get_key.return_value = (
            "eyJpZGVudGl0eSI6IHsiYWNjb3VudF9udW1iZXIiOiAiMTIxMjcyOSIsICJ0eXBlIjogI"
            "lN5c3RlbSIsICJhdXRoX3R5cGUiOiAiY2xhc3NpYy1wcm94eSIsICJzeXN0ZW0iOiB7Im"
        ...
        mock_fetch_url.return_value = {"data": rbac_data}
        mock_request = mm()
        mock_request.path = "/some/path"
        mocked_get_rbac_filters.return_value = {"some return key": "return value to be stored"}
        store_place = {}
        view_helpers.ensure_has_permission(
            ...
            request_metric=mm(),
            exception_metric=mm(),
            rbac_filters=store_place,
        )
        self.assertEquals(store_place, {"some return key": "return value to be stored"})
